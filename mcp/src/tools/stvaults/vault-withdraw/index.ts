...
export function register(server: McpServer, provider: Provider): void {
  server.tool(
    ...
    {
      vault_address: z.string().describe("The vault contract address to withdraw from"),
      recipient: z.string().describe("The address to receive the withdrawn ETH"),
      amount: z.string().describe('Amount of ETH to withdraw (e.g. "1.5")'),
      dry_run: z
        .boolean()
        .default(true)
    ...
    async (args: { vault_address: string; recipient: string; amount: string; dry_run: boolean; chain_id?: number }) => {
      try {
        ...
        const recipientAddr = args.recipient as `0x${string}`;
        ...
        if (args.dry_run) {
          ...
          return success({
            ...
            summary: `Simulation successful. Withdrawing ${args.amount} ETH from vault ${vaultAddr} to ${recipientAddr}. Estimated gas: ${gasEstimate.toString()} (fee: ${formatEther(estimatedFee)} ETH). Set dry_run=false to execute.`,
          });
        }
          ...
          return success({
            ...
            summary:
              receipt.status === "success"
                ? `Successfully withdrew ${args.amount} ETH from vault ${vaultAddr} to ${recipientAddr}. Tx: ${txHash}`
        ...
        return error(
          `Vault withdrawal failed: ${extractErrorMessage(err, "Unknown error")}. Check permissions and RPC connection.`,
        );
      }
    },
  );
