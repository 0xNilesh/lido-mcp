...
export function register(server: McpServer, provider: Provider): void {
  ...
  server.tool(
    "lido_transfer",
    "Transfer stETH, wstETH, or shares to another address",
    {
      token: z
    ...
    async (args: { token: "stETH" | "wstETH" | "shares"; to: string; amount: string; dry_run: boolean }) => {
      try {
        ...
        if (args.token === "stETH") {
          ...
          const sharesBalance = (await provider.publicClient.readContract({
            address: contractAddress,
            abi: transferSharesAbi,
