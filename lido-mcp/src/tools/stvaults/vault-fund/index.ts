...
export function register(server: McpServer, provider: Provider): void {
  server.tool(
    ...
    {
      vault_address: z.string().describe("The vault contract address to fund"),
      amount: z.string().describe('Amount of ETH to deposit (e.g. "1.5")'),
      dry_run: z
        .boolean()
        .default(true)
    ...
    async (args: { vault_address: string; amount: string; dry_run: boolean; chain_id?: number }) => {
      try {
        ...
        if (ethBalance < amountWei) {
          return error(
            `Insufficient ETH balance. Have ${formatEther(ethBalance)} ETH, need ${args.amount} ETH.`,
          );
        }
        ...
        if (args.dry_run) {
          ...
          return success({
            ...
            summary: `Simulation successful. Funding vault ${vaultAddr} with ${args.amount} ETH. Estimated gas: ${gasEstimate.toString()} (fee: ${formatEther(estimatedFee)} ETH). Set dry_run=false to execute.`,
          });
        }
          ...
          return success({
            ...
            summary:
              receipt.status === "success"
                ? `Successfully funded vault ${vaultAddr} with ${args.amount} ETH. Tx: ${txHash}`
        ...
        return error(
          `Vault funding failed: ${extractErrorMessage(err, "Unknown error")}. Check balance and RPC connection.`,
        );
      }
    },
  );
