...
export function register(server: McpServer, provider: Provider): void {
  server.tool(
    "lido_vault_resume_deposits",
    "Resume beacon chain deposits for a Lido staking vault via VaultHub.",
    {
      ...
      try {
        ...
        if (args.dry_run) {
          ...
          return success({
            ...
            summary: `Simulation successful. Resuming beacon chain deposits for vault ${vaultAddr}. Estimated gas: ${gasEstimate.toString()} (fee: ${formatEther(estimatedFee)} ETH). Set dry_run=false to execute.`,
          });
        }
          ...
          return success({
            ...
            summary:
              receipt.status === "success"
                ? `Successfully resumed beacon chain deposits for vault ${vaultAddr}. Tx: ${txHash}`
        ...
        return error(
          `Resume deposits failed: ${extractErrorMessage(err, "Unknown error")}. Check permissions and RPC connection.`,
        );
      }
    },
  );
